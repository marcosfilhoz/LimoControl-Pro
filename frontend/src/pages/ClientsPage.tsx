...
export function ClientsPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((c) => {
      ...
      if (!q) return true;
      const companyName = c.companyId ? companies.find((co) => co.id === c.companyId)?.name ?? "" : "";
      const haystack = [c.name, c.phone ?? "", c.address ?? "", companyName].join(" ").toLowerCase();
      return haystack.includes(q);
    });
  }, [items, companies, searchQuery, activeFilter]);
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <div className="divide-y divide-slate-100">
          ...
          {!loading && filteredItems.length === 0 ? (
            <div className="p-3 text-sm text-slate-600">{items.length === 0 ? "No clients yet." : "No results match the filters."}</div>
          ) : null}
        </div>
      </div>
